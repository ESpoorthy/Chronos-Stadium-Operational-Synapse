    ...
    <main className="min-h-screen bg-background text-foreground overflow-hidden relative">
      ...
      <div className="container mx-auto px-6 py-24 relative z-10">
        <nav className="flex justify-between items-center mb-24">
          ...
          <Link href="/dashboard" className="px-6 py-2 rounded-full border border-primary/30 hover:border-primary transition-colors text-sm uppercase tracking-widest text-primary glowing-border">
            Enter Dashboard
          </Link>
        </nav>
        ...
        <div className="flex flex-col items-center text-center max-w-4xl mx-auto mt-20">
          ...
          >
            <Link href="/dashboard" className="flex items-center gap-2 px-8 py-4 bg-primary text-primary-foreground rounded-full font-medium hover:bg-primary/90 transition-all hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(79,140,255,0.4)]">
              Initialize Operations <ArrowRight className="w-4 h-4" />
            </Link>
          </motion.div>
        </div>
...
function FeatureCard({ icon, title, description }: { icon: React.ReactNode, title: string, description: string }) {
  return (
    <div className="glass-panel p-8 rounded-2xl flex flex-col items-start text-left glowing-border transition-all hover:-translate-y-2 cursor-default group">
